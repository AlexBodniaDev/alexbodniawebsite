  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-background via-background to-muted/30">
      <div
        ...
        style={{
          background: `radial-gradient(800px circle at ${mousePosition.x}% ${mousePosition.y}%, rgba(53, 94, 59, 0.08), transparent 50%)`,
        }}
      />

      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/4 right-1/4 w-40 h-40 rounded-full bg-gradient-to-br from-primary/20 to-accent/30 backdrop-blur-sm animate-float" />
        <div className="absolute bottom-1/3 left-1/5 w-32 h-32 rounded-2xl bg-gradient-to-tr from-accent/30 to-primary/20 backdrop-blur-sm animate-pulse" />
        <div className="absolute top-1/2 right-1/3 w-24 h-24 rounded-full bg-gradient-to-bl from-primary/15 to-accent/25 backdrop-blur-sm animate-glow" />
      </div>
      ...
      <div
        className="absolute bottom-12 left-1/2 transform -translate-x-1/2 cursor-pointer animate-in fade-in slide-in-from-bottom-4 duration-800 delay-1200 hover:scale-110 transition-transform duration-300"
        onClick={scrollToWorks}
      >
